...
- リポジトリ側で lint 設定を再有効化すると IDE 利用時に再びエラーが発生するため、各エージェントは整形ルール（Heading のレベル順序、テーブルの整列など）を手動で維持しつつ、不要な設定追加を避けてください。
- 今後 lint を再導入する場合は `.vscode/settings.json` と CI 設定を調整し、既存ドキュメントへの影響を最小化する手順を整備することが推奨されます。
